...
{
	__m128i m1, m2, vec_sum;
	transpose(mat2);
	int i, j, k;
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			vec_sum = _mm_setzero_si128();
			for (k = 0; k < N-3; k+=4) {
				m1 = _mm_lddqu_si128((__m128i*)(&mat1[i][k]));
				m2 = _mm_lddqu_si128((__m128i*)(&mat2[j][k]));
				vec_sum = _mm_add_epi16(vec_sum, _mm_mullo_epi16(m1, m2));
			}
			__declspec(align(16)) int vec_sum_arr[4];
			_mm_store_si128((__m128i*)(vec_sum_arr), vec_sum);
			int fix = 0;
			for(; k < N; k++){
				fix += mat1[i][k] * mat2[j][k];
			}
			result[i][j] = vec_sum_arr[0] + vec_sum_arr[1] + vec_sum_arr[2] + vec_sum_arr[3] + fix;
		}
	}
	transpose(mat2);
}
